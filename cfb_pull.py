...
def parse_game(game_id, division, date_str):
    ...
    for t in teams:
        ...
        row = {
            ...
            "plays": num(st.get("totalOffensivePlays")),
            "yards_per_play": num(st.get("yardsPerPlay")),
    ...
    for tblock in box.get("players", []) or []:
        ...
        for group in tblock.get("statistics", []) or []:
            ...
            keys = [str(k).strip().lower() for k in (group.get("keys") or group.get("labels") or [])]
...
def enrich_with_cfbd(team_df, date_str):
    ...
    log("Fetching CFBD advanced season stats...")
    data = get_json(
    ...
    if not data:
        log("  CFBD call failed or returned nothing - continuing without it.")
        return team_df
    ...
    for rec in data:
        ...
        adv[rec.get("team")] = {
            "epa_per_play": (off.get("ppa")),
            "success_rate": (off.get("successRate")),
            "explosiveness": (off.get("explosiveness")),
            "line_yards": (off.get("lineYards")),
        }

    hits = 0
    for i, row in team_df.iterrows():
        match = adv.get(row["team"])
        if match:
            hits += 1
            for k, v in match.items():
                team_df.at[i, k] = v
    log(f"  Matched advanced stats for {hits} of {len(team_df)} team rows.")
    return team_df
...
def main():
    ...
    log(f"=== EdgePlay CFB pull for {date_str} ===")
